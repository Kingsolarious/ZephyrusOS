...
get_game_optimization() {
    ...
    local analysis_file="$DATA_DIR/games/$game_analysis.json"
    
    if [ -f "$rec_file" ]; then
        # Use learned recommendation
        local recommended=$(grep -oP '(?<="recommended_pl1":)[0-9]+' "$rec_file" || true)
...
$source_text
Power limit: ${pl1}W
Confidence: ${CONF_PERCENT}%" -t 4000 -i applications-games ||:
}
...
detect_game() {
    ...
    if [ -f /sys/fs/cgroup/gameoverlay/cgroup.procs ] && [ -s /sys/fs/cgroup/gameoverlay/cgroup.procs ]; then
        ...
        if [ -n "$game_pid" ]; then
            local cmdline=$(cat /proc/$game_pid/cmdline ||: | tr '\0' ' ')
            local comm=$(cat /proc/$game_pid/comm 2>/dev/null)
            
            # Extract game name
            if [[ "$cmdline" =~ steam ]]; then
                # Try to find the actual game name from Steam
                for PID in $(cat /sys/fs/cgroup/gameoverlay/cgroup.procs 2>/dev/null || true); do
                    local name=$(cat /proc/$pid/comm || true)
            ...
            if [ "$LAST_GAME_END_TIME" -gt 0 ]; then
                ...
                if [ "$time_since_end" -ge "$COOLDOWN_TIME" ]; then
                    ...
                    if apply_power_limit "$DEFAULT_PL1" "Returning to default after cooldown"; then   
                        NOTIFY-send " Smart Optimizer" "Returned to BALANCED mode (${DEFAULT_PL1}W)" -t 3000 2>/dev/null || true
                        LAST_GAME_END_TIME=0
                    fi
                fi
            fi
