...
case $OPTION in
    1)
        ...
        echo "ok Icon reset to stock KDE (start-here-kde or distributor logo)"
        ;;
    ...
    2)
        ...
        if [ -f "$ROG_LOGO" ]; then
            ...
            echo "ok ROG logo set"   
        else
            echo "fail ROG logo not found at: $ROG_LOGO"
            exit 1   
        fi
        ;;
    ...
    3)
        ...
        echo "ok Panel height set to 48px (icon will be larger)"   
        ;;
