...
if [ -d "$ASUSCTL_DIR" ]; then
    ...
    echo "warn Zephyrus OS repo not found at $ZEPHYRUS_DIR, falling back to pacman..."
...
if [ -d "$ZEPHYRUS_DIR/config/layered-fixes/configs/etc/systemd/system" ]; then
    ...
    echo "ok Zephyrus services installed"
else
    echo "warn Zephyrus OS repo not found, skipping custom services"
fi

echo "=== Disabling conflicting services ==="
sudo systemctl disable nvidia-power-limit.service >/dev/null 2>&1
sudo systemctl disable rapl-tune.service ||:
sudo systemctl disable tuned.service || true
