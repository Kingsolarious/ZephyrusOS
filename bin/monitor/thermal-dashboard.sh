...
show_dashboard() {
  ...
  while true; do
    ...
    echo ""
        ...
        # Current Status
    echo -e "$CYAN CURRENT SYSTEM STATUS${RESET}"
    ...
    echo ""
    printf "  CPU Power Limit:    ${YELLOW}%sW${RESET} (PL1) / %sW (PL2)\n" "$pl1" "$pl2"
    printf "  GPU Power Limit:    ${YELLOW}%sW${RESET}\n" "$gpu_tgp"
    ...
    if [ -d "$DATA_DIR/sessions" ]; then
      ...
      printf "  Sessions Recorded:  ${GREEN}%d${RESET}\n" "$session_count"
      printf "  Games Analyzed:     ${GREEN}%d${RESET}\n" "$game_count"
    ...
    echo ""
        ...
        # Recommendations
    echo -e "${CYAN} CURRENT RECOMMENDATIONS${RESET}"
...
function optimize_game_menu {
  ...
  if [ "$choice" -ge 1 ] && [ "$choice" -lt "$i" ]; then
    ...
    case "$opt" in
      1)
        local rec=$(grep -oP '(?<="recommended_pl1":)[0-9]+' "$DATA_DIR/recommendations/$selected_game_rec.json" 2>/dev/null)
        if [ -n "$rec" ]; then
          echo "Applying $RECW POWER LIMIT..."
      ...
      3)
        echo ""
        cat "$DATA_DIR/games/$selected_game_analysis.json"
        ;;
      4)
...
EOF
    ...
    echo "ok Created optimized launcher: $launcher_file"
...
main_menu() {
    while true; do
        ...
        echo "  5)   Configure Learning"
        echo "  6)   Start Learning Daemon"
        echo "  7)   Stop Learning Daemon"
        echo "  8)  Export Data"
        echo "  9) Exit"
        echo ""
