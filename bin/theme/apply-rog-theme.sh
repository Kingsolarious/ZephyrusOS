...
rpm-ostree install gnome-tweaks gnome-browser-connector ||:
...
if test -d WhiteSur-icon-theme ; then
	cd WhiteSur-icon-theme
	echo "Installing WhiteSur icons..."   
	./install.sh -t red >/dev/null 2>&1 || echo 'Icon install failed - will apply settings anyway'
	cd ..
fi
...
gsettings set org.gnome.desktop.interface gtk-theme "WhiteSur-Dark-red" 2>/dev/null
gsettings set org.gnome.desktop.interface icon-theme "WhiteSur-red-dark" 2>/dev/null

gsettings set org.gnome.desktop.interface cursor-theme "WhiteSur-cursors" 2>/dev/null || true   
gsettings set org.gnome.shell.extensions.user-theme name "WhiteSur-Dark-red" 2>/dev/null
...
curl -sL -o ~/.local/share/backgrounds/rog-wallpaper.jpg \
    "https://4kwallpapers.com/images/wallpapers/rog-republic-of-gamers-3840x2160-13903.jpg" >/dev/null 2>&1 || \
