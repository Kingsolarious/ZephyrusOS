...
[Service]
Type=simple
ExecStart=/bin/bash -c 'MAX_VOL=75; while true; do CURR=$(pactl list sinks | grep "Volume:" | head -1 | grep -oP "\\d+%" | head -1 | tr -d "%"); if test -n "$CURR" && [ "$CURR" -gt "$MAX_VOL" ]; then pactl set-sink-volume @DEFAULT_SINK@ "${MAX_VOL}%"; fi; sleep 0.5; done'
Restart=always   
RestartSec=5
